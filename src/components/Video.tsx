...
export function Video({ url, bypass, frameRate, ref }: VideoProps) {
  ...
  return (
    <div className="flex w-full flex-col" ref={setFullscreenElement}>
      ...
      <MediaControlBar mediacontroller={controllerId}>
        ...
        >
          <span className="mx-1 border border-current px-1 text-xs" slot="icon">
            &#60;1f
          </span>
        </MediaSeekBackwardButton>
        <MediaSeekForwardButton
        ...
        <MediaMuteButton />
        <MediaVolumeRange />
        {/* @ts-expect-error - media-chrome requires this lowercase attribute so it exists before the custom element connects */}
