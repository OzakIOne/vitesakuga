...
export const fetchUserPostsEffect = Effect.fn("fetchUserPosts")(function* (
  ...
  yield* Effect.logInfo(`Fetching posts for user ${userId} (page ${page})`);
  ...
  const userInfo = yield* Effect.tryPromise({
    ...
    catch: (error) => new Error(`User ${userId} not found: ${String(error)}`),
  });

  yield* Effect.logDebug("Building query...");
  let query = db.selectFrom("posts").selectAll().where("userId", "=", userId);
