...
const debouncedSearch = debounce((query: string) => fetchResults(query), {
  wait: 300,
});
...
const [debouncedValue, setValue] = useDebouncedState(initialValue, {
  wait: 300,
});
...
const rateLimitedFn = useRateLimitedCallback(apiCall, {
  limit: 5,
  window: 1000,
});
```
