...
Inventaire des fonctionnalités visibles de ViteSakuga (clone de Sakugabooru), vérifié contre le code au **2026-09-12**.
...
Le fil public `/news` présente les annonces de ViteSakuga de la plus récente à la plus ancienne. Chaque article possède sa page `/news/<slug>`, un titre, un résumé, une date et un corps Markdown. Liens depuis la navigation desktop et le menu mobile. Les articles sont conservés dans `src/content/news/`, enregistrés dans `src/lib/news/news.ts` et publiés avec les déploiements ; pas d’éditeur admin ni de base de données. Voir [Publication des actualités](./news.md).
...
| **Post aléatoire**          | `/random` choisit un post au hasard puis redirige vers son détail ; le lien est disponible dans la navigation desktop et dans le menu mobile                                                                                           | `src/routes/random.tsx`, `src/lib/posts/posts.service.ts`                                                                                                                         |
