...
`src/lib/db/schema/sakuga.schema.ts` : `tags`, `post_tags`, `tag_follows`, `posts`, `post_images`, `post_votes`, `post_reports`, `playlists`, `playlist_posts`, `comments`, `comment_mentions`, `points_ledger`, `promotion_reviews`, `notifications`, `post_edits`, `post_edit_approvals`, `video_revisions`.
